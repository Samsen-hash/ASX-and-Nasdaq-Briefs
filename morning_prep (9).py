...
INDEX_TICKERS = {
    "ASX 200": "^AXJO",
    "S&P 500": "SPY",          # ETF - Yahoo more reliable than ^GSPC
    "Nasdaq 100": "QQQ",        # ETF - Yahoo more reliable than ^NDX
    "Dow Jones": "DIA",         # ETF - Yahoo more reliable than ^DJI
    "VIX": "^VIX",
    "Nikkei 225": "^N225",
    "Hang Seng": "^HSI",
    "AUD/USD": "AUDUSD=X",
...
def fetch_price_row(ticker: str, include_series: bool = False) -> dict:
    try:
        t = yf.Ticker(ticker)
        hist = t.history(period="10d")  # widened from 5d to handle long weekends
        if hist.empty:
        ...
        # Weekend-aware staleness: on Monday, US/HK markets have Friday data (3 days old)
        # which is fresh, not stale. Calculate business days, not calendar days.
        weekday = today.weekday()  # Mon=0, Sun=6
        if weekday == 0:        # Monday: up to 3 calendar days old is fresh
            stale_threshold = 3
        elif weekday == 6:      # Sunday: up to 2 days old is fresh
            stale_threshold = 2
        else:                   # Tue-Sat: 1 day old is fresh
            stale_threshold = 1
        ...
        if len(hist) >= 2:
            ...
            pct = 0
        result = {"last": round(last, 2), "pct": round(pct, 2),
                  "status": "ok" if days_old <= stale_threshold else "stale",
                  "days_old": days_old}
        if include_series:
            result["series"] = [float(c) for c in hist["Close"].tolist()]
        return result
...
def fetch_watchlist_with_thesis(items):
    return [{"ticker": t.replace(".AX", ""), "raw_ticker": t, "thesis": th, **fetch_price_row(t, include_series=True)}
            for t, th in items]
...
def build_price_table(prices, keys):
    ...
    for k in keys:
        if k in prices:
            ...
            rows.append(f"<tr><td style='padding:6px 7px;border-bottom:1px solid var(--border);'>{k}</td><td style='padding:6px 7px;border-bottom:1px solid var(--border);text-align:right;font-variant-numeric:tabular-nums;width:80px;'>{last}</td><td style='padding:6px 7px;border-bottom:1px solid var(--border);text-align:right;font-variant-numeric:tabular-nums;width:90px;'>{price_cell(v)}</td></tr>")
    return f"<table style='width:100%;border-collapse:collapse;font-size:12.5px;'><tr><th style='text-align:left;font-size:10px;text-transform:uppercase;color:var(--muted);letter-spacing:0.5px;padding:5px 7px;border-bottom:1px solid var(--border);'>Market</th><th style='text-align:right;font-size:10px;text-transform:uppercase;color:var(--muted);letter-spacing:0.5px;padding:5px 7px;border-bottom:1px solid var(--border);width:80px;'>Level</th><th style='text-align:right;font-size:10px;text-transform:uppercase;color:var(--muted);letter-spacing:0.5px;padding:5px 7px;border-bottom:1px solid var(--border);width:90px;'>Chg</th></tr>{''.join(rows)}</table>"
...
def build_sparkline(series: list[float], width: int = 80, height: int = 22) -> str:
    """Render a tiny inline SVG sparkline for a price series."""
    ...
    # Colour by start vs end
    colour = "#0a7a0a" if series[-1] >= series[0] else "#c62828"
    # Area fill
    area_pts = [f"0,{height}"] + pts + [f"{width},{height}"]
    return f"""<svg width="{width}" height="{height}" viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg" style="display:block;">
  <polygon points="{' '.join(area_pts)}" fill="{colour}" opacity="0.12" />
  <polyline points="{' '.join(pts)}" fill="none" stroke="{colour}" stroke-width="1.5" />
</svg>"""


def render_thesis_bullets(thesis: str) -> str:
    """Split a thesis string into bullet points on comma/semicolon."""
    # Split on commas or semicolons, clean up, capitalise first letter of each
    parts = re.split(r"[,;]", thesis)
    parts = [p.strip() for p in parts if p.strip()]
    if not parts:
        return ""
    bullets = "".join(f'<li>{p[0].upper() + p[1:] if p else ""}</li>' for p in parts)
    return f'<ul class="thesis-list">{bullets}</ul>'


def count_news_hits(watchlist: list[dict], feeds: dict) -> list[dict]:
    """Count news mentions of each watchlist ticker across all feeds.
    Matches ticker symbol (word boundary) and company-name hints from thesis."""
    all_items = []
    for section_items in feeds.values():
    ...
    for r in watchlist:
        ticker = r["ticker"]
        hits = []
        # Use regex with word boundaries so "CBA" doesn't match "CBAX"
...
def build_news_hits_table(hits: list[dict]) -> str:
    """Render the news hits companion table."""
    # Only show stocks with at least 1 hit, cap at 12 rows
    active = [h for h in hits if h["count"] > 0][:12]
    if not active:
        return '<p style="color:#888;font-size:11.5px;padding:10px;">No watchlist names in overnight news feed.</p>'

    rows = []
    for h in active:
        count = h["count"]
        badge_class = "hot" if count >= 4 else "warm" if count >= 2 else "cool"
        latest = h["latest"]
        headline_html = ""
        if latest:
            title = latest.get("title", "")[:85]
            link = latest.get("link", "")
            if link:
                headline_html = f'<a href="{link}" target="_blank">{title}</a>'
            else:
                headline_html = title
        rows.append(f"""
        <tr>
          <td class='nh-ticker'>{h['ticker']}</td>
          <td class='nh-count'><span class='nh-badge {badge_class}'>{count}</span></td>
          <td class='nh-headline'>{headline_html}</td>
        </tr>
        """)
    return f"""<table class='nh-tbl'>
      <thead>
        <tr>
          <th>Ticker</th>
...
def build_watchlist_table(rows):
    ...
    for r in rows:
        ...
        thesis_html = render_thesis_bullets(r.get("thesis", ""))
        tr.append(f"""
        <tr>
          <td class='wl-ticker'><b>{r['ticker']}</b></td>
          <td class='wl-last'>{last}</td>
          <td class='wl-pct' style='color:{colour_pct(pct)};'>{pct_str}</td>
          <td class='wl-trend'>{sparkline}</td>
          <td class='wl-thesis'>{thesis_html}</td>
        </tr>
        """)
    return f"""<table class='wl-tbl'>
      <thead>
        <tr>
          <th class='wl-ticker'>Ticker</th>
          <th class='wl-last'>Last</th>
          <th class='wl-pct'>%</th>
          <th class='wl-trend'>5D Trend</th>
          <th class='wl-thesis'>Thesis</th>
        </tr>
      </thead>
      <tbody>{''.join(tr)}</tbody>
    </table>"""
...
def build_html(data):
    ...
    asx_mkt = ["ASX 200", "S&P 500", "Nasdaq 100", "Dow Jones", "VIX", "Nikkei 225", "Hang Seng"]
    asx_mac = ["AUD/USD", "US 10Y Yield", "US 2Y Yield", "Gold", "WTI Crude", "Iron Ore (62%)", "Copper"]
    ndx_mkt = ["Nasdaq 100", "S&P 500", "Dow Jones", "VIX", "Nikkei 225"]
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .tbl th:nth-child(1) {{ width: 45%; }}
  .tbl th:nth-child(2) {{ width: 25%; }}
  .tbl th:nth-child(3) {{ width: 30%; }}
  .tbl td {{ padding: 6px 7px; border-bottom: 1px solid var(--border); font-variant-numeric: tabular-nums; }}
  .num {{ text-align: right; font-variant-numeric: tabular-nums; }}

  /* Watchlist grid layout */
  .wl-grid {{ display: grid; grid-template-columns: 2fr 1fr; gap: 18px; margin-bottom: 20px; }}
  .wl-alone {{ max-width: 860px; }}
  .wl-side-title {{ font-size: 10.5px; text-transform: uppercase; letter-spacing: 1px; color: var(--brand); font-weight: 600; margin-bottom: 8px; padding-bottom: 6px; border-bottom: 1px solid var(--border); }}

  /* Watchlist-specific table */
  .wl-tbl {{ width: 100%; border-collapse: collapse; font-size: 11.5px; table-layout: fixed; }}
  .wl-tbl th {{ text-align: left; font-size: 9.5px; text-transform: uppercase; color: var(--muted); letter-spacing: 0.5px; padding: 5px 7px; border-bottom: 1px solid var(--border); }}
  .wl-tbl td {{ padding: 6px 7px; border-bottom: 1px solid var(--border); vertical-align: middle; }}
  .wl-tbl th.wl-ticker, .wl-tbl td.wl-ticker {{ width: 50px; }}
  .wl-tbl th.wl-last, .wl-tbl td.wl-last {{ width: 60px; text-align: right; font-variant-numeric: tabular-nums; }}
  .wl-tbl th.wl-pct, .wl-tbl td.wl-pct {{ width: 60px; text-align: right; font-weight: 600; font-variant-numeric: tabular-nums; }}
  .wl-tbl th.wl-trend, .wl-tbl td.wl-trend {{ width: 90px; }}
  .wl-tbl th.wl-thesis, .wl-tbl td.wl-thesis {{ padding-left: 12px; }}
  .thesis-list {{ margin: 0; padding: 0 0 0 14px; font-size: 10.5px; color: var(--muted); line-height: 1.4; }}
  .thesis-list li {{ margin: 0; }}

  /* News hits table */
  .nh-tbl {{ width: 100%; border-collapse: collapse; font-size: 11.5px; }}
  .nh-tbl th {{ text-align: left; font-size: 9.5px; text-transform: uppercase; color: var(--muted); letter-spacing: 0.5px; padding: 5px 7px; border-bottom: 1px solid var(--border); }}
  .nh-tbl td {{ padding: 6px 7px; border-bottom: 1px solid var(--border); vertical-align: middle; }}
  .nh-tbl td.nh-ticker {{ font-weight: 700; width: 50px; color: var(--brand); }}
  .nh-tbl td.nh-count {{ width: 40px; text-align: center; font-variant-numeric: tabular-nums; }}
  .nh-badge {{ display: inline-block; padding: 1px 6px; border-radius: 8px; font-size: 10px; font-weight: 700; min-width: 20px; }}
  .nh-badge.hot {{ background: #fef0f0; color: #c62828; }}
  .nh-badge.warm {{ background: #fff8e1; color: #a07000; }}
  .nh-badge.cool {{ background: #f0f4f8; color: #666; }}
  .nh-headline {{ font-size: 10.5px; color: var(--muted); line-height: 1.4; }}
  .nh-headline a {{ color: var(--muted); text-decoration: none; }}
  .nh-headline a:hover {{ color: var(--brand); }}
...
<div class="container">
  ...
  <div id="tab-asx" class="tab-content active">
    ...
    {'<div class="wl-grid"><div class="card wl-main">' + build_watchlist_table(data["asx_watchlist"]) + '</div><div class="card wl-side"><div class="wl-side-title">📰 Overnight News Hits</div>' + build_news_hits_table(data["asx_news_hits"]) + '</div></div>' if any(h["count"] > 0 for h in data["asx_news_hits"]) else '<div class="card wl-alone">' + build_watchlist_table(data["asx_watchlist"]) + '</div>'}
    <div class="grid3">
  ...
  <div id="tab-ndx" class="tab-content">
    ...
    <h2>Watchlist + Thesis</h2>
    {'<div class="wl-grid"><div class="card wl-main">' + build_watchlist_table(data["ndx_watchlist"]) + '</div><div class="card wl-side"><div class="wl-side-title">📰 Overnight News Hits</div>' + build_news_hits_table(data["ndx_news_hits"]) + '</div></div>' if any(h["count"] > 0 for h in data["ndx_news_hits"]) else '<div class="card wl-alone">' + build_watchlist_table(data["ndx_watchlist"]) + '</div>'}
    <div class="grid3">
...
def main():
    ...
    print("→ Counting news hits per watchlist ticker...")
    asx_news_hits = count_news_hits(asx_watchlist, asx_feeds)
    ndx_news_hits = count_news_hits(ndx_watchlist, ndx_feeds)
    ...
    data = {"prices": prices, "asx_watchlist": asx_watchlist, "ndx_watchlist": ndx_watchlist,
            ...
            "asx_news_hits": asx_news_hits, "ndx_news_hits": ndx_news_hits,
